...
MAX_REINTENTOS = 3
MAX_WORKERS = 10  # Threads paralelos


# Entidades que NO se deben encriptar (24 entidades)
ENTIDADES_EXCLUIDAS = {
...
# Todas las entidades posibles (000-999), EXCEPTO las excluidas
ENTIDADES_PERMITIDAS = {f'{i:03d}' for i in range(1000)} - ENTIDADES_EXCLUIDAS
...
def leer_cuentas(archivo: str) -> List[Dict[str, str]]:
    ...
    with open(archivo, 'r', encoding='ISO-8859-1') as f:
        ...
        for row in reader:
            ...
            # Filtrar solo entidades permitidas
            if cod in ENTIDADES_PERMITIDAS:
                cuentas.append({
                    'cuenta': row['cuenta'],
                    'nombre': row['nombre'],
                    'dni': row['dni'],
    ...
    print(f"{len(cuentas)} registros a procesar (excluidos: {excluidos})")
    return cuentas
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Error: Especifica el numero de parte (1-5)")
        print("Uso: python encriptar_parte.py 1")
        sys.exit(1)

    parte_num = sys.argv[1]

    print("=" * 60)
    print(f"    ENCRIPTACION - PARTE {parte_num}")
    print("=" * 60)

    # Leer archivo de la parte correspondiente
    archivo_parte = f"todos_encontrados_parte{parte_num}.txt"
    cuentas = leer_cuentas(archivo_parte)
    ...
    # Guardar resultados finales con nombre por parte
    archivo_salida = f"cuentas_encriptadas_parte{parte_num}.txt"
    guardar_resultados(resultados, archivo_salida, modo='final')

    # Guardar archivo de errores
    archivo_errores = f"cuentas_con_error_parte{parte_num}.txt"
    guardar_errores(resultados, archivo_errores)
